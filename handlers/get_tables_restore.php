<?php
// Script untuk mendapatkan daftar tabel dari database untuk restore
session_start();
...
// Buat log file
$log_file = $log_dir . '/get_tables_restore.log';
...
try {
    ...
    if (!empty($tables)) {
        ...
        $html .= '</div>';
        $html .= '<label class="form-label">Pilih Tabel untuk Restore</label>';
        ...
        $html .= '</table>';
        $html .= '</div>';
        $html .= '<div class="form-text text-warning">Perhatian: Hanya tabel yang dipilih yang akan direstore. Struktur tabel harus sudah ada di database tujuan.</div>';
    } else {
